...
const onboarding = require('../services/instagramOnboardingService');
const { BusinessUser } = require('../../models');

const fail = (res, err, context) => {
  console.error(`[instagramOnboardingController.${context}] Error:`, err.message);
...
module.exports = {
  ...
  getConfig: (req, res) => {
    ...
    return res.status(200).json({
      ...
      ...(cfg.configured
        ? {}
        : { message: 'Set META_APP_ID and META_INSTAGRAM_CONFIG_ID (or META_CONFIG_ID) in .env to enable Embedded Signup.' }),
    });
  },
   ...
   * exchanges the code for a Page access token, finds the linked Instagram
   * Professional account, subscribes this app to the Page, and stores it
